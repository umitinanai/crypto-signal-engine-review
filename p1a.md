<!-- p1a.md — Part 1/6 of all_code_part1.md — 30 files -->
...
<!--   - adaptive/challenger.py -->
<!--   - adaptive/cycle.py -->
<!--   - adaptive/decision.py -->
<!--   - adaptive/drift_signal.py -->
<!--   - adaptive/evaluation.py -->
<!--   - adaptive/policy.py -->
<!--   - adaptive/rollback.py -->
<!--   - adaptive/scheduler.py -->
<!--   - adaptive/shadow.py -->
<!--   - adaptive/store.py -->
<!--   - adaptive/symbol_score.py -->
<!--   - adaptive/windows.py -->
<!--   - crypto_signal_engine/__init__.py -->
<!--   - crypto_signal_engine/agents/__init__.py -->
<!--   - crypto_signal_engine/agents/base.py -->
<!--   - crypto_signal_engine/agents/context.py -->
<!--   - crypto_signal_engine/agents/market_structure.py -->
<!--   - crypto_signal_engine/agents/order_book.py -->
<!--   - crypto_signal_engine/agents/quant.py -->
<!--   - crypto_signal_engine/agents/regime.py -->
...
=== FILE: crypto_signal_engine/agents/__init__.py ===


=== FILE: crypto_signal_engine/agents/base.py ===
"""
Agent temel yardımcıları.

Kural (Faz 4 Bölüm 4): agent'lar bağımsızdır. Bir agent yalnızca
`AgentContext` ve Phase 3 `FeatureSnapshot`'ları tüketebilir; başka bir
agent'ı, `AgentEvidence`'ını, `ConsensusResult`'ı, `RiskAssessment`'ı veya
nihai `Signal`'ı ASLA tüketemez.

Kural (Bölüm 14): tüm agent'lar deterministic, finite, [-1,1] aralığında
skor üretir; girdi snapshot'larını mutate etmez, güncel zaman kullanmaz,
...
def clamp(value: float, lower: float, upper: float) -> float:
    """Bir değeri [lower, upper] aralığına sıkıştırır (basit, paylaşılan yardımcı)."""
    return max(lower, min(upper, value))


def require_features(
    snapshot: FeatureSnapshot, required: list[str], agent_name: str, timeframe_label: str
) -> dict[str, float]:
    """`snapshot`'tan `required` feature'ların TÜMÜNÜ okur; herhangi biri
    eksikse (`snapshot.get(name) is None`) `AgentInputError` fırlatır —
    sahte sıfır/NaN İLE DOLDURULMAZ (Bölüm 24)."""
    values: dict[str, float] = {}
    missing: list[str] = []
    for name in required:
        ...
        if value is None:
            ...
            values[name] = value
    if missing:
        raise AgentInputError(agent_name, timeframe_label, missing)
    return values


class Agent(Protocol):
    """Standart agent arayüzü (RegimeAgent hariç — o `RegimeAgentOutput` döner)."""

    def evaluate(self, context: AgentContext) -> AgentEvidence: ...


=== FILE: crypto_signal_engine/agents/context.py ===
"""
AgentContext.

Kural (Faz 4 Bölüm 9): Phase 3 `FeatureSnapshot.context_id` opsiyoneldir ve
Phase 3, ayrı ayrı üretilen timeframe snapshot'larının ortak bir context_id
paylaşacağını GARANTİ ETMEZ. Phase 3 bu amaçla DEĞİŞTİRİLMEZ — bunun
yerine burada küçük, immutable bir Faz 4 değerlendirme bağlamı tanımlanır.

Kural (Bölüm 10): context_id DETERMİNİSTİK üretilir — UUID4, random,
hash(), wall-clock, mutable counter, process identity KULLANILMAZ. SHA-256
ile sabit sıradaki kanonik bir temsilden üretilir.

Kural (Bölüm 8 — no-look-ahead HARD BLOCKER): `build_agent_context()`,
`FeatureHistoryStore.as_of()` (PUBLIC, no-look-ahead garantili API)
KULLANIR — asla `latest()` veya private store içseli okunmaz.
"""
...
_CONTEXT_ID_SCHEMA_VERSION = "phase4-context-v1"
_CONTEXT_ID_DIGEST_LENGTH = 32  # hex karakter (16 byte) — kısaltılmış ama kararlı
...
def compute_context_id(
    symbol: str, as_of: datetime, m1_as_of: datetime, m5_as_of: datetime, m15_as_of: datetime, h1_as_of: datetime
) -> str:
    """Deterministik context_id üretir.

    Aynı girdiler HER ZAMAN aynı context_id'yi üretir; katılan
    snapshot'lardan herhangi birinin `as_of` değeri değişirse context_id de
    değişir (Bölüm 10 gereksinimi). SHA-256, stdlib `hashlib` kullanılır —
    UUID/random/hash()/wall-clock/mutable counter YOKTUR.
    """
    canonical = "|".join([
        _CONTEXT_ID_SCHEMA_VERSION,
    ...
    return digest[:_CONTEXT_ID_DIGEST_LENGTH]


@dataclass(frozen=True)
class AgentContext:
    """Dört timeframe'in hizalı, no-look-ahead-garantili, immutable anlık görüntüsü.

    Invariant'lar (Bölüm 9):
    - symbol normalize edilir
    - as_of UTC-aware olmalı
    - her snapshot'ın timeframe'i BEKLENEN timeframe ile eşleşmeli (m1.timeframe==M1, vb.)
    - her snapshot'ın symbol'ü context symbol'üyle eşleşmeli (mixed-symbol reddi)
    - her snapshot.as_of <= context.as_of (gelecekten snapshot reddi)
    - context_id boş olmayan bir str olmalı
    """
    ...
    def __post_init__(self) -> None:
        ...
        expected = {
            "m1": (self.m1, Timeframe.M1),
            "m5": (self.m5, Timeframe.M5),
            "m15": (self.m15, Timeframe.M15),
            "h1": (self.h1, Timeframe.H1),
        }
        for label, (snapshot, expected_timeframe) in expected.items():
            if not isinstance(snapshot, FeatureSnapshot):
                raise TypeError(f"{label} bir FeatureSnapshot olmalı, alınan: {type(snapshot)!r}")
            if snapshot.timeframe != expected_timeframe:
                raise ValueError(
                    f"{label} snapshot'ının timeframe'i {expected_timeframe} olmalı, "
                    f"alınan: {snapshot.timeframe}"
                )
            if snapshot.symbol != canonical_symbol:
                raise ValueError(
                    f"mixed-symbol AgentContext reddedildi: {label}.symbol={snapshot.symbol} "
                    f"!= context.symbol={canonical_symbol}"
                )
            if snapshot.as_of > self.as_of:
                raise ValueError(
                    f"gelecekten snapshot reddedildi: {label}.as_of ({snapshot.as_of}) > "
                    f"context.as_of ({self.as_of})"
                )

    def snapshot_for(self, timeframe: Timeframe) -> FeatureSnapshot:
        mapping = {Timeframe.M1: self.m1, Timeframe.M5: self.m5, Timeframe.M15: self.m15, Timeframe.H1: self.h1}
        return mapping[timeframe]


def build_agent_context(history: FeatureHistoryStore, symbol: str, as_of: datetime) -> AgentContext:
    """`FeatureHistoryStore.as_of()` (no-look-ahead garantili PUBLIC API)
    kullanarak dört timeframe'in hizalı `AgentContext`'ini inşa eder.

    Gerekli timeframe'lerden HERHANGİ BİRİ için `as_of`'ta veya ONDAN ÖNCE
    hiç snapshot yoksa, AÇIKÇA başarısız olur (`AgentInputError`) — sahte
    nötr bir değerlendirme UYDURULMAZ, gelecekteki bir snapshot'a
    DÜŞÜLMEZ, başka bir timeframe'in verisi İKAME EDİLMEZ (Bölüm 11).
    """
    canonical_symbol = normalize_symbol(symbol)
    require_utc_aware(as_of, "as_of")

    snapshots: dict[Timeframe, FeatureSnapshot | None] = {
        Timeframe.M1: history.as_of(canonical_symbol, Timeframe.M1, as_of),
        Timeframe.M5: history.as_of(canonical_symbol, Timeframe.M5, as_of),
        Timeframe.M15: history.as_of(canonical_symbol, Timeframe.M15, as_of),
        Timeframe.H1: history.as_of(canonical_symbol, Timeframe.H1, as_of),
    }
    missing = [tf.value for tf, snap in snapshots.items() if snap is None]
    if missing:
        raise AgentInputError("AgentContextBuilder", ",".join(missing), missing)

    context_id = compute_context_id(
        symbol=canonical_symbol, as_of=as_of,
        m1_as_of=snapshots[Timeframe.M1].as_of, m5_as_of=snapshots[Timeframe.M5].as_of,
        m15_as_of=snapshots[Timeframe.M15].as_of, h1_as_of=snapshots[Timeframe.H1].as_of,
    )

    return AgentContext(
        symbol=canonical_symbol, as_of=as_of, context_id=context_id,
        m1=snapshots[Timeframe.M1], m5=snapshots[Timeframe.M5],
        m15=snapshots[Timeframe.M15], h1=snapshots[Timeframe.H1],
    )
